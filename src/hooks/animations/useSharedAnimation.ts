...
export const useSharedAnimation = () => {
  ...
  const createTimeline = useCallback(
    (targets: AnimationTarget[], config: TimelineConfig = {}) => {
  ...
  const getPresetAnimation = useCallback(
  ...
  // Inside useSharedAnimation.ts
  const animateCards = useCallback(
    (cards: NodeListOf<Element> | Element[], scroller: HTMLElement | null) => {
      if (!scroller) return;

      const viewportHeight = window.innerHeight;
      const cardHeight = viewportHeight / 3; // Each card takes up 1/3 of viewport
      const centerPoint = viewportHeight / 2; // Center of viewport
      ...
      cards.forEach((card) => {
        ...
        ScrollTrigger.create({
          ...
          onUpdate: (self) => {
            ...
            const distanceFromCenter = Math.abs(cardCenter - centerPoint);
